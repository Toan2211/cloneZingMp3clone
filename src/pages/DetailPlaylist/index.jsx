...
import { setCurrentIndexSong, setCurrentIndexSongRandom, setCurrentTime, setInfoSongPlayer, setIsPlay, setPlayListId, setPlaylistRandom, setPlaylistSong, setSongId, setSrcAudio } from 'redux/audioSlice';
DetailPlaylist.propTypes = {
    
};

function DetailPlaylist(props) {
    ...
    const handlePlaySong = (playlistId, playListSong, song) => {
        ...
        if (song.streamingStatus === 1 && song.isWorldWide)
        {
            ...
            playListCanPlay = playListSong.filter((item) => item.streamingStatus === 1 && item.isWorldWide)
            dispatch(setPlaylistSong(playListCanPlay))
            dispatch(setPlaylistRandom(shuffle(playListCanPlay)))
            dispatch(setPlayListId(playlistId))
            dispatch(setCurrentIndexSong(playListCanPlay.indexOf(song)))
    ...
    useEffect(() => {
        (async () => {
            try {
                const dataFromServer = await audioApi.getDetailPlayList(playlistId)
            ...
            <div className = "detail-playlist-info">
                <div className="detail-playlist-avatar">
                    ...
                    <div className = "detail-playlist-play">
                        <FontAwesomeIcon icon = {faPlayCircle} className = "detail-playlist-play-btn"/>
                    </div>
                </div>
                <div className="detail-playlist-description">
                    <h3 className="detail-playlist-title">{data.title}</h3>
                    ...
                    <p className="liked">{data.like} người yêu thích</p>
                </div>
            </div>
            <div className="detail-playlist-content">
                <div className="detail-playlist-short-description">
                    <p className="detail-playlist-short-description-content">{data.sortDescription}</p>
                    {
                        data.song.items.map((song, index) => (
                            <SongItem key = {song.encodeId} index = {index} song = {song} onClick = {() => handlePlaySong(playlistId, data.song.items, song)}/>
                        ))
                    }
                </div>
            </div>
